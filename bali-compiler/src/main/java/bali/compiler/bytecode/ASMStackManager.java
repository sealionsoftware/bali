...
public class ASMStackManager implements Opcodes {
	...
	private DepthCounter counter = new DepthCounter();
	...
	private void execute(Invocation statement, MethodVisitor v) {
		...
		for (Value value : statement.getArguments()){
			push(value, v);
			argumentClasses.add(value.getValueTypeName());
		}
		v.visitMethodInsn(INVOKEVIRTUAL,
				converter.getInternalName(statement.getTarget().getValueTypeName()),
				statement.getMethod(),
				converter.getMethodDescriptor(statement.getValueTypeName(), argumentClasses));
	}
	...
	private void unwrapBoolean(MethodVisitor v){
		v.visitFieldInsn(GETFIELD, "bali/Boolean", "value", "Z");
	}
	...
	public void push(NumberLiteralValue value, MethodVisitor v) {
		...
		String internalName = converter.getInternalName(value.getValueTypeName());
	...
	public void push(BooleanLiteralValue value, MethodVisitor v) {
		...
		v.visitFieldInsn(GETSTATIC, converter.getInternalName(value.getValueTypeName()), bool ? "TRUE" : "FALSE", converter.getTypeDescriptor(value.getValueTypeName()));
	}
	...
	public void push(ListLiteralValue value, MethodVisitor v) {
		String implName = converter.getInternalName(value.getValueTypeName());
	...
	public void push(ReferenceValue value, MethodVisitor v) {
		switch(value.getScope()){
			case STATIC: {
				v.visitFieldInsn(GETSTATIC, converter.getInternalName(value.getHostClass()), value.getName(), converter.getTypeDescriptor(value.getValueTypeName()));
			} break;
			case FIELD: {
				v.visitVarInsn(ALOAD, 0);
				v.visitFieldInsn(GETFIELD, converter.getInternalName(value.getHostClass()), value.getName(), converter.getTypeDescriptor(value.getValueTypeName()));
			} break;
			case VARIABLE: {
	...
	public void push(ConstructionValue value, MethodVisitor v) {
		...
		v.visitMethodInsn(INVOKESPECIAL, internalName, "<init>", converter.getMethodDescriptor(null, new ArrayList<String>())); //TODO: actual constructor method descriptors
	}
	...
	public void push(Invocation value, MethodVisitor v) {
		...
		for (Value argumentValue : value.getArguments()){
			push(argumentValue, v);
			argumentClasses.add(argumentValue.getValueTypeName());
		}
		v.visitMethodInsn(INVOKEVIRTUAL,
				converter.getInternalName(value.getTarget().getValueTypeName()),
				value.getMethod(),
				converter.getMethodDescriptor(value.getValueTypeName(), argumentClasses));
	}
